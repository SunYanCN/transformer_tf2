...
def load_examples():
    ...
    examples, metadata = tfds.load(CONFIG['data']['origin'], with_info = True, as_supervised = True)
...
def build_tokenizer(examples):
    ...
    if not os.path.exists(en_vocab_path_prefix + '.subwords'):
        ...
        tokenizer_en = tfds.deprecated.text.SubwordTextEncoder.load_from_file(filename_prefix = en_vocab_path_prefix)
...
def gen_dataset(batch_size = 64):
    ...
    def tf_encode(pt, en):
        return tf.py_function(encode_text, [pt, en], [tf.int64, tf.int64])
...
if __name__ == '__main__':
    # sample_string = 'why tf2 is so different from tf1.'
    # tokenizer_en, tokenizer_pt = build_tokenizer(sample_string)
